...
import { findLearnedFacts, loadLearnedMemory } from '@/lib/memory/learnedMemory';
...
export async function runAgent({ mode, prompt, sql, selectedTables = [], tableAck = false, llmRuntime }) {
  ...
  if (normalizedMode === 'sql') {
    ...
    if (missingDb.length) {
      ...
      const learnedMemory = await loadLearnedMemory();
      const learnedFacts = findLearnedFacts(learnedMemory, prompt);
      ...
      if (learnedFacts.length) {
        const learnedTables = learnedFacts.map((fact) => fact.table).filter(Boolean);
        tablePlan = {
          recommendedTables: Array.from(new Set([...(learnedTables || []), ...(tablePlan?.recommendedTables || [])])).slice(0, 8),
          finalTables: Array.from(new Set([...(learnedTables || []), ...(tablePlan?.finalTables || [])])).slice(0, 8),
