...
function askIfSQLNeedsBounds(mode, prompt = '') {
  if (mode !== 'sql') return null;
  const q = prompt.toLowerCase();
  const hasRange = /\b(today|yesterday|last|between|from|to|week|month|year|limit)\b/.test(q);
  // SQL mode can safely default row limit and still generate query. Do not block on this.
...
export async function runPreflight({ mode, prompt, sql = '', selectedTables = [] }) {
  const normalizedMode = (mode || 'architecture').toLowerCase();
  const intent = inferIntent({ mode: normalizedMode, prompt });
  const contexts = await searchKnowledge(prompt, normalizedMode, 5, intent.domains);
  ...
  if (normalizedMode === 'sql') {
    ...
    const hasUserIdSignal = /\b(marketplace_user_id|user_id|customer_id|marketplace user id|user id)\b/i.test(prompt);
    if (!hasUserIdSignal) {
      questions.push('Confirm user identifier column name (example: marketplace_user_id, user_id, customer_id).');
    }
