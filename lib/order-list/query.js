...
function buildWhereClause(plan, requestScope) {
  ...
  for (const filter of plan.filters) {
    switch (filter.field) {
      case 'customer_id':
        if (filter.operator === 'in' && Array.isArray(filter.value) && filter.value.length) {
          where.push('j.customer_id IN (?)');
          values.push(filter.value.map((value) => Number(value)));
        } else {
      ...
      case 'marketplace_user_id':
        if (filter.operator === 'in' && Array.isArray(filter.value) && filter.value.length) {
          where.push('j.marketplace_user_id IN (?)');
          values.push(filter.value.map((value) => Number(value)));
        } else {
          where.push('j.marketplace_user_id = ?');
          values.push(Number(filter.value));
      ...
      case 'created_by':
        if (filter.operator !== 'in' || !Array.isArray(filter.value) || !filter.value.length) {
          throw new Error('created_by supports only in([...]) for Phase 1.');
        }
        where.push('j.created_by IN (?)');
        values.push(filter.value.map((value) => Number(value)));
        break;
      case 'total_amount':
      ...
      case 'creation_datetime':
        ...
        throw new Error('creation_datetime supports between/gte/gt/lte/lt for Phase 1.');
        break;
      case 'store_name':
...
export function summarizeAppliedFilters(plan, context) {
  ...
  for (const filter of plan.filters) {
    switch (filter.field) {
      case 'customer_id':
        labels.push(Array.isArray(filter.value) ? `customer id in (${filter.value.join(', ')})` : `customer id = ${filter.value}`);
        break;
      case 'job_id':
        labels.push(Array.isArray(filter.value) ? `job id in (${filter.value.join(', ')})` : `job id = ${filter.value}`);
        break;
      case 'order_id':
        labels.push(Array.isArray(filter.value) ? `order id in (${filter.value.join(', ')})` : `order id = ${filter.value}`);
        break;
      case 'marketplace_user_id':
        labels.push(Array.isArray(filter.value) ? `marketplace user id in (${filter.value.join(', ')})` : `marketplace user id = ${filter.value}`);
        break;
      case 'job_status':
        if (Array.isArray(filter.value)) {
          labels.push(`status in (${filter.value.map((value) => statusReverse[value] || value).join(', ')})`);
        } else {
      ...
      case 'created_by':
        if (Array.isArray(filter.value)) {
          if (filter.value.includes(12)) {
            labels.push('source = web');
          } else if (filter.value.includes(10) || filter.value.includes(14)) {
            labels.push('source = android');
          } else if (filter.value.includes(11) || filter.value.includes(13)) {
            labels.push('source = ios');
          }
        }
        break;
      case 'total_amount':
      ...
      case 'creation_datetime':
        if (filter.operator === 'between') {
          ...
          labels.push(`created after ${filter.value}`);
        } else if (filter.operator === 'lte' || filter.operator === 'lt') {
          labels.push(`created before ${filter.value}`);
        }
        break;
      case 'store_name':
